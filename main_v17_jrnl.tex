...
\end{array}
\right.
\end{equation} We express this as a transition probability matrix $P_{p}$, whose entries in the $p^{th}$ subset of rows are $p_{i,j}(u_{p})$.  
...
\end{equation} Here, $\iota=[I,...,I]^{T}$ is a matrix consisting of $\rho$ copies of the identity matrix, $P$ is a block diagonal matrix containing all the matrices $P_{p}$ along its diagonal, and $F=[(F_{1})^{T},...,(F_{\rho})^{T}]^{T}$ is a matrix consisting of each matrix $F_{p}$ structured as defined. %Here, $\Lambda = [\lambda,\hdots, \boldsymbol{\lambda_{p}}]^{T}$ is a vector containing $\rho$ copies of vectors $\boldsymbol{\lambda_{p}}$. Each vector $\boldsymbol{\lambda_{p}}$ contains the current state values, $\lambda(x_{i})$, as its components.

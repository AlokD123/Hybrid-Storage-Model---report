...
Let $x_{i}$ be the state at time $t$. Let $\chi$ be the set of states that are reached from any state $x_{i}\in X$ with any control $u_{p}\in U$ and any demand $w_{k}\in W$ by the state transition $f(x_{i},u_{p},w_{k})$. Let $x_{j}$ be the $j^{\text{th}}$ state in the set $\chi$, where $j\in \{1,...,M^{2}NP\}$. If $x_{j}=f(x_{i},u_{p},w_{k})$ is the next state and $x_{j}\not\in X$, we apply interpolation according to \cite[p.~319]{MultiLinInterp} to determine the value function at $x_{j}$:
...
First, note that no interpolation is needed for the demand, $L(t)$, because it is drawn from a discrete distribution. Hence, we use bilinear interpolation for the weightings. Let $a_{l}$ be the $l^{\text{th}}$ value of energy $E_{1}$, $b_{m}$ be the $m^{\text{th}}$ value of energy $E_{2}$, and $L_{k}$ be the $k^{\text{th}}$ demand in the set $W$. Then, we represent $x_{i}=x_{l,m,k}$ in terms of its coordinates as $[a_{l},b_{m},L_{k}]^{T}$. Note that $x_{i}$ and $x_{l,m,k}$ refer to the same state because we use the linear index $i$ for the set $X$, which is equivalent to standard indexing with $(l,m,k)$. Similarly to $x_{i}$, we represent $x_{j}$ as $[a,b,L']^{T}$. Here $a\in\mathbb{R}$ and $b\in\mathbb{R}$ are, respectively, the energies $E_{1}$ and $E_{2}$ associated with the $j^{\text{th}}$ state, and $L'$ is a demand in the set $W$.

Then, the interpolation weights are $q_{i',j}=q_{l,m,k,j}$, where:
%\begin{displaymath}q_{l,m,j}=
%\left\{
%\begin{array}{l}
%\Big(1-\frac{a-a_{l}}{a_{l+1}-a_{l}}\Big)\Big(1-\frac{b-b_{m}}{b_{m+1}-b_{m}}\Big)\\ \text{if} \hspace{0.1cm} a_{l}\leq a_{j}<a_{l+1},b_{m}\leq b<b_{m+1}, L_{k}=L_{n}\\\\
...
\end{array}
\right.
\end{displaymath} Note that $q_{i',j}$ and $q_{l,m,k,j}$ refer to the same quantity for the same reasons that $x_{i}$ is the same quantity as $x_{l,m,k}$.

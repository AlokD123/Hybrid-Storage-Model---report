...
Let $X$ be the set of values the discretized state, $x(t)$, can take. As the state evolves with each iteration of the DP, the subsequent state, $x(t+1)=f(x(t),u(t),w(t))$, may not lie in $X$. We use multilinear interpolation according to \cite[p.~319]{MultiLinInterp} to determine the value function at the state $x(t+1)\not\in X$. First, we define four sets: $W$ as the set of random demands, indexed by $k\in\{1,...,M\}$; $X_{1}$ as the set of discrete energy levels $E_{1}$, indexed by $l \in\{1,...,N_{1}\}$; $X_{2}$ as the set of discrete energy levels $E_{2}$, indexed by $m \in\{1,...,N_{2}\}$; and $U$ as the set of controls, indexed by $p\in\{1,...,\rho\}$. By the above definition, $X=X_{1}\times X_{2}\times W$. We index $X$ by the linear index $i\in\{1,...,NM\}$, or equivalently by the triplet of indices $(l,m,k)$. We use subscripts to index the elements in each set, e.g., $x_{i}$ is the $i^{\text{th}}$ element in set $X$.
...
\begin{equation} \label{eq:InterpMap}
	J_{t+1}(\overline{x}_{j})=\sum_{\delta}^{MN}q_{\delta,j}J_{t+1}(x_{\delta})
\end{equation} Here, $\delta\in\{1,...,NM\}$ such that $x_{\delta}$ is any vector in $X$, and $q_{\delta,j}$ are interpolation weights.%In our numerical examples, we use multilinear interpolation.

We note that no interpolation is needed for the demand, $L(t)$, because it is drawn from a discrete distribution. Hence, we use bilinear interpolation for the weightings. Let $x_{\delta}$ be any arbitrary state in $X$, such that $\delta\in\{1,...,NM\}$ and $\delta$ may or may not equal $i$. Let $a_{l}$ be the $l^{\text{th}}$ value of energy $E_{1}$, $b_{m}$ be the $m^{\text{th}}$ value of energy $E_{2}$, and $L_{k}$ be the $k^{\text{th}}$ demand in the set $W$. Then, we represent $x_{\delta}=x_{l,m,k}$ in terms of its coordinates as $[a_{l},b_{m},L_{k}]^{T}$. Note that $x_{\delta}$ and $x_{l,m,k}$ refer to the same state because we use the linear index $\delta$ for the set $X$, which is equivalent to standard indexing with $(l,m,k)$. Similarly to $x_{\delta}$, we represent $\overline{x}_{j}$ as $[a,b,L']^{T}$. Here $a\in\mathbb{R}$ and $b\in\mathbb{R}$ are, respectively, the energies $E_{1}$ and $E_{2}$ associated with the $j^{\text{th}}$ state in $\chi$, and $L'$ is a demand in the set $W$.

Then, the interpolation weights are $q_{\delta,j}=q_{l,m,k,j}$, where:
\begin{displaymath}q_{l,m,k,j}=
%\left\{
%\begin{array}{l}
\Big(1-\frac{a-a_{l}}{a_{l+1}-a_{l}}\Big)\Big(1-\frac{b-b_{m}}{b_{m+1}-b_{m}}\Big)%\\ \text{if} \hspace{0.1cm} a_{l}\leq a_{j}<a_{l+1},b_{m}\leq b<b_{m+1}, L'=L_{k}
%\\\\0		   \hspace{4.8cm} \text{otherwise}
%\end{array}
%\right.
\end{displaymath} if $a_{l}\leq a_{j}<a_{l+1}$, $b_{m}\leq b<b_{m+1}$, and $L'=L_{k}$. Note that $q_{\delta,j}$ and $q_{l,m,k,j}$ refer to the same quantity for the same reasons that $x_{\delta}$ is the same quantity as $x_{l,m,k}$. For clarity, we suppress adding a subscript $p$ to $q_{l,m,k,j}$ that would indicate its dependence on the control $u_{p}$.

%\begin{displaymath}q_{l,m,k,j}=
%\left\{
%\begin{array}{l}
%\Big(1-\frac{a-a_{l}}{a_{l+1}-a_{l}}\Big) \hspace{0.7cm} \text{if} \hspace{0.1cm} a_{l}\leq a< a_{l+1},L'=L_{k},\\ \times \Big(1-\frac{b-b_{m}}{b_{m+1}-b_{m}}\Big) \hspace{1.8cm}b_{m}\leq b< b_{m+1}\\\\
...
%\end{array}
%\right.
%\end{displaymath} Here, $a_{j}$ and $b_{j}$ are coordinates of the point $x_{j}\not\in X$.

To compute the optimal policy, we can encode the interpolation scheme in a constant matrix $F_{p}$ for control $u_{p}$. The entry $(j,\delta)$ of that matrix is as follows:
\begin{displaymath}
F_{j,\delta,p}=
\left\{
\begin{array}{l}
1		   \hspace{1cm} \text{if} \hspace{0.2cm}\overline{x}_{j}=x_{\delta}\\
q_{l,m,k,j}\hspace{0.1cm} \text{if} \hspace{0.1cm} a_{l}\leq a<a_{l+1}, b_{m}\leq b<b_{m+1},L'=L_{k}\\
...
We express our model as a Markov chain to enable a linear programming formulation. Let $p_{i,\gamma}(u_{p})=P(x_{\gamma}| x_{i},u_{p})$ be the probability of moving from $x_{i}\in X$ to $\overline{x}_{\gamma}\in \chi$ by applying control $u_{p}\in U$. Note that $\gamma\in\{1,...M^{2}N\rho\}$. For $p_{i,\gamma}(u_{p})$ to be non-zero, the next state $x_{j}$ must be accessible from the current state, i.e., $x_{\gamma}=f(x_{i},u_{p},w_{k})$. The only random variable in $f(x_{i},u_{p},w_{k})$ is $w_{k}\in W$, so we may express this instead as a probability of $w_{k}$, which is $P(w_{k} | x_{i},u_{p})$. The transition probability is then:
...
\end{equation} Here, $\iota=[I,...,I]^{T}$ is a matrix consisting of $\rho$ copies of the identity matrix and $\overline{P}=[(P_{1}F_{1})^{T},...,(P_{\rho}F_{\rho})^{T}]^{T}$ is a matrix consisting of matrices $P_{p}F_{p}$, structured as defined. %Here, $\Lambda = [\lambda,\hdots, \boldsymbol{\lambda_{p}}]^{T}$ is a vector containing $\rho$ copies of vectors $\boldsymbol{\lambda_{p}}$. Each vector $\boldsymbol{\lambda_{p}}$ contains the current state values, $\lambda(x_{i})$, as its components.
...
We must use interpolation to implement this policy when $x\not\in X$. Let $x_{j}=f(x_{i}, u, w)\not\in X$ be the next state, and let $u'(x_{j})$ be the interpolated suboptimal control to apply in this state. We represent state $x_{j}$ as $[a,b,L']^{T}$ and state $x_{l,m,k}\in X$ as $[a_{l},b_{m},L_{k}]$, just as in Section II. Then, we can approximate the optimal control in state $x_{i}$ using bilinear interpolation as:

\begin{equation}
    u'(x_{j})=\sum_{l,m,k}q_{l,m,k,j}u^{*}(x_{l,m,k})
\end{equation} %Here $q_{l,m,j}$ is as defined above.
...
The approximate value function has the form $\Phi r$, where $\Phi \in \mathbb{R}^{MN\times R}$ is termed the design matrix and $r\in \mathbb{R}^R$ is a vector of weights. By choosing $R$ to be lower than the number of entries in the value function, we reduce the size of the linear program. The $q^{\text{th}}$ column vector of $\Phi$ is a basis function denoted by $\phi_{q}$. In this section, we discuss methods to generate basis functions for the two-storage problem.
